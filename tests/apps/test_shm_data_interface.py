...
def _make_di(height=4, width=4, shm_buffer_size=1, shm_name_prefix="shared_frame_memory"):
    """Helper: create a data interface with a mock input device and config."""
    device = SimpleNamespace(height=height, width=width)
    config = {"shm_buffer_size": shm_buffer_size, "shm_name_prefix": shm_name_prefix}
...
class TestConfigureSampling:
    ...
    def test_creates_shm_segments(self):
        di = _make_di(height=64, width=64, shm_buffer_size=3)
